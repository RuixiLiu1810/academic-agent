...
import { loadThemeFromPath, type Theme } from "./theme-loader.js";
...
export class DefaultResourceLoader implements ResourceLoader {
	...
	async reload(): Promise<void> {
		...
		const getEnabledResources = (resources: Array<{ path: string; enabled: boolean; metadata: PathMetadata }>) => {
		...
		): string[] => getEnabledResources(resources).map((resource) => resource.path);
		...
		for (const extensionPath of this.additionalExtensionPaths) {
			if (isLocalPath(extensionPath) && !existsSync(extensionPath)) {
				extensionsResult.errors.push({
					path: extensionPath,
					error: `Extension path does not exist: ${extensionPath}`,
				});
			}
		}
		...
		for (const skillPath of this.additionalSkillPaths) {
			if (
				isLocalPath(skillPath) &&
				!existsSync(skillPath) &&
				!this.skillDiagnostics.some((diagnostic) => diagnostic.path === skillPath)
			) {
				this.skillDiagnostics.push({ type: "error", message: "Skill path does not exist", path: skillPath });
			}
		}
		...
		for (const promptPath of this.additionalPromptTemplatePaths) {
			if (
				isLocalPath(promptPath) &&
				!existsSync(promptPath) &&
				!this.promptDiagnostics.some((diagnostic) => diagnostic.path === promptPath)
			) {
		...
		for (const themePath of this.additionalThemePaths) {
			if (!existsSync(themePath) && !this.themeDiagnostics.some((diagnostic) => diagnostic.path === themePath)) {
				this.themeDiagnostics.push({ type: "error", message: "Theme path does not exist", path: themePath });
			}
		}
		...
		const appendSource = this.discoverAppendSystemPromptFile();
		const appendSources = this.appendSystemPromptSource ?? (appendSource ? [appendSource] : []);
	...
	private normalizeExtensionPaths(
		...
		return entries.map((entry) => ({ path: this.resolveResourcePath(entry.path), metadata: entry.metadata }));
	}
	...
	private updateSkillsFromPaths(skillPaths: string[], metadataByPath?: Map<string, PathMetadata>): void {
		...
		if (this.noSkills && skillPaths.length === 0) {
			...
			skillsResult = loadSkills({ cwd: this.cwd, agentDir: this.agentDir, skillPaths, includeDefaults: false });
		}
	...
	private mergePaths(primary: string[], additional: string[]): string[] {
		...
		for (const candidate of [...primary, ...additional]) {
			const resolvedPath = this.resolveResourcePath(candidate);
			const canonicalPath = canonicalizePath(resolvedPath);
			if (seen.has(canonicalPath)) continue;
			seen.add(canonicalPath);
			merged.push(resolvedPath);
		}
		return merged;
	}
	...
	private loadThemes(
		...
		if (includeDefaults) {
			for (const dir of [join(this.agentDir, "themes"), join(this.cwd, CONFIG_DIR_NAME, "themes")]) {
		...
		for (const themePath of paths) {
			...
			try {
				...
				if (stats.isDirectory()) this.loadThemesFromDir(resolvedPath, themes, diagnostics);
				else if (stats.isFile() && resolvedPath.endsWith(".json"))
					this.loadThemeFromFile(resolvedPath, themes, diagnostics);
				else diagnostics.push({ type: "warning", message: "theme path is not a json file", path: resolvedPath });
			} catch (error) {
	...
	private loadThemesFromDir(dir: string, themes: Theme[], diagnostics: ResourceDiagnostic[]): void {
		...
		try {
			...
			for (const entry of entries) {
				...
				if (!isFile || !entry.name.endsWith(".json")) continue;
		...
		for (const extension of extensions) {
			for (const toolName of extension.tools.keys()) {
				...
				if (existingOwner && existingOwner !== extension.path) {
					conflicts.push({ path: extension.path, message: `Tool "${toolName}" conflicts with ${existingOwner}` });
				} else {
